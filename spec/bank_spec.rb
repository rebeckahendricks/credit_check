...
RSpec.describe Bank do
  ...
  it 'knows a transaction is invalid when credit card is invalid' do
    bank = Bank.new('Wells Fargo')
    credit_check = CreditCheck.new('4034007106512380', 15_000)
  ...
  it 'knows a transactions is invalid when the amount is over the limit' do
    bank = Bank.new('Wells Fargo')
    credit_check = CreditCheck.new('5541808923795240', 15_000)
    expect(bank.valid_transaction?(20_000, credit_check)).to be false
  end

  it 'knows when a transaction is valid' do
    bank = Bank.new('Wells Fargo')
    credit_check = CreditCheck.new('5541808923795240', 15_000)
    expect(bank.valid_transaction?(11_000, credit_check)).to be true
  end
end
